...
- Long-term squat strength goal: work the primary squat-pattern lift toward roughly 2x bodyweight over time -- this is a strong predictor of vertical jump ability. See squat-pattern strength progress below for whether back squat should still be the primary bilateral squat-pattern lift, and whether a bilateral squat-pattern lift (back squat, front squat, trap bar deadlift, or similar) needs to appear on a fixed number of training days this week or exercise selection can vary freely day to day -- when a fixed count is called for, 1 day is enough during a deload week, and don't force it on a 1-day/week program. If they're well below the benchmark and recovery/adherence support it, don't be shy about progressing squat-pattern loads assertively. Once the full 2x-bodyweight goal has been met, prioritize power/reactive work over pure strength since further squat gains alone won't move the needle much more.
$$),
