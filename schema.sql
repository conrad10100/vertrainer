...
- Long-term squat strength goal: work the primary squat-pattern lift toward roughly 2x bodyweight over time -- this is a strong predictor of vertical jump ability. Back squat should stay the primary bilateral squat-pattern lift until that goal is met (see squat-pattern strength progress below). While the goal is unmet, include a bilateral squat-pattern lift (back squat, front squat, trap bar deadlift, or similar) on at least 2 different training days this week -- 1 is enough during a deload week, and don't force this on a 1-day/week program. If they're well below the benchmark and recovery/adherence support it, don't be shy about progressing squat-pattern loads assertively. Once the goal has been met, shift primary emphasis away from back squat toward front squat, box squat, and half/partial squat variations, and prioritize power/reactive work over pure strength since further squat gains alone won't move the needle much more.
$$),
